...
def create_agent(project_id, location="global", yaml_file=DEFAULT_YAML_FILE):
    ...
    display_name = "BigQuery Information Agent"
    description = "Answer questions about the BigQuery compute (jobs) and storage using the Information_Schema tables"
...
def update_config(agent_yaml_path=DEFAULT_OUTPUT_CONFIG, template_yaml_path=DEFAULT_YAML_FILE):
    ...
    camel_config = to_camel(agent_dict)

    creds, _ = google.auth.default()
    creds.refresh(Request())

    url = f"https://geminidataanalytics.googleapis.com/v1beta/{agent_name}?updateMask=displayName,description,labels,dataAnalyticsAgent.publishedContext"
    headers = {
    ...
    try:
        print("Sending update request via REST API...")
        response = requests.patch(url, headers=headers, json=camel_config)
        response.raise_for_status()
        
        # Poll for LRO completion if it returned an operation
        res_json = response.json()
        if 'name' in res_json and '/operations/' in res_json['name']:
            op_name = res_json['name']
            op_url = f"https://geminidataanalytics.googleapis.com/v1beta/{op_name}"
            print(f"Operation in progress: {op_name}")
            while True:
                time.sleep(2)
